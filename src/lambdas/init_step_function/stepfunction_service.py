...
class StepFunctionTriggerService:
    ...
    def start_step_functions(self, payload: Any) -> str:
        ...
        email = (
            payload_dict.get("email", "unknown")
        ...
        try:
            ...
            logger.info(
                "Step Functions execution started successfully. ARN: %s", execution_arn
            )
            return execution_arn
        except Exception as e:
            logger.error(
                "Error starting Step Functions execution: %s", e, exc_info=True
            )
            raise
